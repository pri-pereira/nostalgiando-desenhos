        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="relative aspect-video w-full overflow-hidden rounded-3xl border border-border/80 bg-black shadow-2xl">
              {!episode ? (
                <div className="flex h-full w-full flex-col items-center justify-center text-muted-foreground p-6 text-center">
                  ...
                  <p className="text-sm mt-2 opacity-70">Se for um desenho do catálogo, aguarde o carregamento do Internet Archive.</p>
                </div>
              ) : episode.videoUrl ? (
                ...
                <>
                  ...
                  {/* PAYWALL OVERLAY — aparece após 20 segundos para não-logados */}
                  {showPaywall && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/85 backdrop-blur-md animate-in fade-in duration-500">
                      <div className="max-w-md text-center p-6">
                        ...
                        <p className="text-sm text-muted-foreground mb-6 leading-relaxed">
                          Faça login ou crie sua conta <strong className="text-foreground">gratuita</strong> para continuar assistindo e ter acesso ao catálogo completo do Nostalgiando Desenhos.
                        </p>

                        <div className="flex flex-col sm:flex-row gap-3 justify-center">
                          <Link
                            to="/login"
                            className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-full bg-gradient-to-r from-primary to-amber-600 text-primary-foreground font-bold text-sm shadow-[0_0_20px_rgba(217,119,6,0.4)] hover:shadow-[0_0_30px_rgba(217,119,6,0.6)] hover:scale-[1.03] active:scale-[0.97] transition-all"
                          >
                        ...
                        <p className="text-[11px] text-muted-foreground/50 mt-5">
                          É rápido, gratuito e você desbloqueia todos os episódios.
                        </p>
                      </div>
                    </div>
                  )}
            ...
            <div className="mt-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground font-semibold">{show.year}</span>
                    <span className="text-muted-foreground">•</span>
                    <span className="rounded-md bg-accent/20 px-2.5 py-0.5 text-xs font-bold text-accent">
                      Dublado
                    </span>
                ...
                {/* BOTÕES DE CONTROLE RESPONSIVOS: ANTERIOR / PRÓXIMO / MINHA LISTA / COMPARTILHAR */}
                <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2">
                  <button
                    onClick={handlePrev}
                    disabled={current === 0}
                    className="h-11 inline-flex items-center justify-center gap-1.5 rounded-2xl border border-border bg-secondary/60 px-4 text-sm font-bold text-foreground transition-colors hover:border-primary hover:text-primary disabled:opacity-40 disabled:pointer-events-none active:scale-95"
                  >
                  ...
                  <button
                    onClick={handleNext}
                    disabled={current === episodesList.length - 1}
                    className="h-11 inline-flex items-center justify-center gap-1.5 rounded-2xl border border-border bg-secondary/60 px-4 text-sm font-bold text-foreground transition-colors hover:border-primary hover:text-primary disabled:opacity-40 disabled:pointer-events-none active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className={`h-11 inline-flex items-center justify-center gap-2 rounded-2xl border px-4 text-sm font-bold transition-all active:scale-95 ${
                      inList
                  ...
                  <button
                    onClick={handleShare}
                    className="h-11 inline-flex items-center justify-center gap-2 rounded-2xl border border-border bg-secondary/60 px-4 text-sm font-bold text-foreground transition-colors hover:border-primary hover:text-primary active:scale-95"
                  >
              ...
              {/* SINOPSE DO EPISÓDIO */}
              <div className="mt-5 rounded-3xl border border-border/70 bg-card p-5 sm:p-6 shadow-sm">
              ...
              {/* SOBRE O DESENHO */}
              <div className="mt-4 flex flex-col sm:flex-row sm:items-center gap-4 rounded-3xl border border-border/60 bg-secondary/30 p-5">
                <img
                  src={show.poster}
                  alt={show.title}
                  className="h-28 w-20 shrink-0 rounded-2xl object-cover shadow-md mx-auto sm:mx-0"
                />
